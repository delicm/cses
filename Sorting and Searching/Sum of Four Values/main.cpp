...
bool check4(int pf, int k, pair <int, int> a[])
{
    for(int i = pf; i >= 3; i--)
    {
        if(check3(i, k-a[i].first, a)) {p4=a[i].second; return 1;}
    }
    return 0;
}
...
int main()
{
    ...
    if(check4(n-1, x, a)==0) {cout << "IMPOSSIBLE"; return 0;}
